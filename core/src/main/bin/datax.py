...
DEFAULT_JVM = "-Xms64m -Xmx1g -XX:+HeapDumpOnOutOfMemoryError -Dfile.encoding=UTF-8 -XX:HeapDumpPath={}".format(
    ADDAX_HOME)
...
def getOptionParser():
    ...
    parser.add_option("--filename", metavar="<filename>", dest="filename", action="store", default="-1",
                                  help="Logfilename is filename.")
...
def buildStartCommand(options, args):
    ...
    if options.filename != '-1' and options.filename != -1:
        jobParams = ("-Daddax.log=%s -Dlog.file.name=%s.log") % (options.logdir,options.filename)
    else:
        print(options.logdir)
        jobParams = ("-Daddax.log=%s -Dlog.file.name=addax_%s_%s_%s.log") % (options.logdir, jobFilename, curr_time, os.getpid())
